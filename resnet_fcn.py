...
def inference(x, is_training, 
              num_classes=1000,
              num_blocks=[3, 4, 6, 3],  # defaults to 50-layers network
...
def upscore_layer(x, shape, num_classes, name, ksize, stride):
    ...
    with tf.variable_scope(name):
        ...
        output_shape = tf.stack(new_shape)                  # output shape
        f_shape = [ksize, ksize, num_classes, in_features]  # shape of devonv_filter
        num_input = ksize * ksize * in_features / stride
        stddev = (2 / num_input)**0.5
        weights = get_deconv_filter(f_shape)  # get deconv filter
...
def block(x, c):
    ...
    # True, filters_internal*4 filters are outputed. filters_internal is how many filters
    # the 3x3 convs output internally.
    m = 4 if c['bottleneck'] else 1
    filters_out = m * c['block_filters_internal']  # output channels

    shortcut = x  # branch 1

    c['conv_filters_out'] = c['block_filters_internal']  # internal channels

    if c['bottleneck']:
        with tf.variable_scope('a'):
            # squeeze channels to block_filters_internal
            # by conv 1x1 filters
            c['ksize'] = 1
        ...
        with tf.variable_scope('b'):
            # conv 3x3, channels no change
            x = conv(x, c)
            x = bn(x, c)
            x = activation(x)

        with tf.variable_scope('c'):
            # conv 1x1, make output channels back to original
        ...
        with tf.variable_scope('A'):
            # conv 3x3, channels to block_filters_internal
            c['stride'] = c['block_stride']
            assert c['ksize'] == 3
            x = conv(x, c)
            x = bn(x, c)
        ...
        with tf.variable_scope('B'):
            # conv 3x3, output channels equal to internal
            c['conv_filters_out'] = filters_out 
            assert c['ksize'] == 3
            assert c['stride'] == 1
            x = conv(x, c)
    ...
    with tf.variable_scope('shortcut'):
        if filters_out != filters_in or c['block_stride'] != 1:
            # make sure shortcut and x have same channels.
            c['ksize'] = 1
            c['stride'] = c['block_stride']
            c['conv_filters_out'] = filters_out  
...
def bn(x, c):
    x_shape = x.get_shape()
    params_shape = x_shape[-1:]  # get num of channels

    if c['use_bias']:
        bias = _get_variable('bias', params_shape, initializer=tf.zeros_initializer)
        return x + bias

    axis = list(range(len(x_shape) - 1))  # normalize along dims except channels
    ...
    mean, variance = tf.nn.moments(x, axis)  # along axes = [0, 1, 2] compute mean and var
    update_moving_mean = moving_averages.assign_moving_average(moving_mean, mean, BN_DECAY, zero_debias=False)
    update_moving_variance = moving_averages.assign_moving_average(moving_variance, variance, BN_DECAY, zero_debias=False)
    ...
    x = tf.nn.batch_normalization(x, mean, variance, beta, gamma, BN_EPSILON)  # gamma*(x-mean/sqrt(variance+epsilon)) + beta
...
def fc(x, c):
    # x example: [None, 4, 4]-> tf.flatten ->[None, 16]
    # so here use x.get_shape[1]
...
def _get_variable(name,
    ...
    to do weight decay and add to resnet collection"""
    if weight_decay > 0:
